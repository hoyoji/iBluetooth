...
public class MoneyAccountSearchListFragment extends HyjUserExpandableListFragment {
...
//		((HyjSimpleExpandableListAdapter)getListView().getExpandableListAdapter()).setOnFetchMoreListener(this);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (item.getItemId() == R.id.searchListFragment_action_search) {
			...
			return true;
		}  else if (item.getItemId() == R.id.mainActivity_action_money_addnew) {
...
//		super.onCreateLoader(groupPos, arg1);
		Object loader;
		if (groupPos < 0) { // 这个是分类
			loader = new MoneyAccountSearchGroupListLoader(getActivity(), arg1);
		} else {
			loader = new MoneyAccountSearchChildListLoader(getActivity(), arg1);
		}
	...
	public void onLoadFinished(Loader loader, Object list) {
		...
		if (loader.getId() < 0) {
			...
			this.setFooterLoadFinished(((MoneyAccountSearchGroupListLoader)loader).hasMoreData());
		} else {
