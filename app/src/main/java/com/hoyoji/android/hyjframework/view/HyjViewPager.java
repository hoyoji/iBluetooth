...
public class HyjViewPager extends ViewPager {
	 ...
	 */
	final private static int DEFAULT_OVERSCROLL_ANIMATION_DURATION = 500;
	...
	private void init(AttributeSet attrs) {
		...
		mOverscrollTranslation = a.getDimension(
				R.styleable.BounceBackViewPager_overscroll_translation,
				DEFAULT_OVERSCROLL_TRANSLATION);
		mOverscrollAnimationDuration = a.getInt(
				R.styleable.BounceBackViewPager_overscroll_animation_duration,
	...
	protected boolean getChildStaticTransformation(View child, Transformation t) {
		...
		if (mOverscrollEffect.isOverscrolling() && isFirstOrLast) {
			...
			final float translateX = (float) mOverscrollTranslation
					* (mOverscrollEffect.mOverscroll > 0 ? Math.min(
							mOverscrollEffect.mOverscroll, 1) : Math.max(
